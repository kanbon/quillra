...
export function EditorSyncModal({ controller }: { controller: SyncController }) {
  const { phase, status, error, acceptMerge, acceptDiscard, dismiss } = controller;
  const behindInfo = status?.state === "behind_with_local_changes" ? status : null;
  const visible =
    Boolean(behindInfo) && (phase === "needs_choice" || phase === "working" || phase === "failed");
  ...
  return (
    <Modal
      ...
      ariaLabel="Your team pushed new changes"
 ...
 */
export function EditorSyncToast({
  toast,
  error = false,
}: { toast: string | null; error?: boolean }) {
  if (!toast) return null;
  return (
    <div className="pointer-events-none fixed right-4 top-16 z-50">
      <div
        className={cn(
          "rounded-lg border px-3 py-2 text-[12.5px] shadow-md",
          error
