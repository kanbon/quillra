 ...
 *
 * This is the v1 of the migration skill. It's deliberately compact: enough
 * to give the agent the right mental model of Astro without turning into a
 * 2000-line doc dump. Grow it as real migrations
...
- Don't introduce a CSS framework that wasn't there. If the source used Tailwind, wire it via \`@tailwindcss/vite\` (the current approach for Tailwind v4 with Astro). **Do not add \`@astrojs/tailwind\`, that integration is deprecated and never had a 6.x release; adding it will break \`npm install\`.** Add \`@tailwindcss/vite\` and \`tailwindcss\` (v4) as deps, then register the plugin in \`astro.config.mjs\` via \`import tailwindcss from '@tailwindcss/vite'; export default defineConfig({ vite: { plugins: [tailwindcss()] } })\`, and import the Tailwind entry CSS once from a global stylesheet. If it used CSS modules, port them as \`.module.css\` next to each component. If it was plain CSS, use scoped Astro \`<style>\` blocks.
- **Always pin dependencies to versions that exist on npm.** Before you write a \`^X.Y.Z\` into package.json, check the package's own Astro integration docs or \`npm view <pkg> versions\`, don't guess major versions. A non-existent version makes \`npm install\` fail with \`ETARGET\` and leaves the site with no dev server. \`@astrojs/react\` is currently v4 (not v5/v6), \`@astrojs/sitemap\` is v3, \`astro\` is v5.
